...
class BaseBuffer(ABC):
    ...
    @abstractmethod
    def _get_samples(
        self, batch_inds: np.ndarray, env: Optional[gym.Env] = None
    ...
    @staticmethod
    def _normalize_reward(reward: np.ndarray, env: Optional[gym.Env] = None) -> np.ndarray:
        if env is not None:
...
class ReplayBuffer(BaseBuffer):
    ...
    def sample(self, batch_size: int, env: Optional[gym.Env] = None) -> ReplayBufferSamples:
        """
...
class DictReplayBuffer(ReplayBuffer):
    ...
    def __init__(
        ...
        self.observations = {
            key: np.zeros(
                (self.buffer_size, self.n_envs, *_obs_shape), dtype=observation_space[key].dtype
            )
        ...
        if psutil is not None:
            ...
            total_memory_usage: float = (
                obs_nbytes + self.actions.nbytes + self.rewards.nbytes + self.dones.nbytes
            )
            if not optimize_memory_usage:
                next_obs_nbytes = 0
                for _, obs in self.observations.items():
