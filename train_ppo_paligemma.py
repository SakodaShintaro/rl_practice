...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--off_wandb", action="store_true", default=False)
...
class Agent:
    ...
    def select_action(self, reward: float, state: np.ndarray) -> tuple:
        ...
        if len(self.r_list) < self.seq_len:
            padding_size = self.seq_len - len(self.r_list)
            for _ in range(padding_size):
                pad_r = np.zeros_like(self.r_list[0])
                pad_s = np.zeros_like(self.s_list[0])
                self.r_list.insert(0, pad_r)
                self.s_list.insert(0, pad_s)

        assert len(self.r_list) == self.seq_len
        assert len(self.s_list) == self.seq_len

        print(f"{self.s_list[-1].shape=}, {self.s_list[-2].shape=}")

        model_inputs = (
            self.processor(text=self.prompt, images=[self.s_list], return_tensors="pt")
            .to(torch.bfloat16)
            .to(device)
        )
        ...
        with torch.inference_mode():
            generation = self.net.generate(**model_inputs, max_new_tokens=100, do_sample=False)
            generation = generation[0][input_len:]
            print(f"{generation.shape=}")
...
if __name__ == "__main__":
    ...
    wandb.init(project="rl_practice", config=vars(args), name="PPO_paligemma", save_code=True)
